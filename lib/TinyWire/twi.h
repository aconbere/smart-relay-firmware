 ...
 *
 * This library gives the functionality to provide a nearly full I2C
 * communication with both master and slave mode through the USI hardware of an
 * ATTiny (since it has no dedicated hardware for this purpose) The master
 * functionality is done directly (not in an ISR) and in case of a requestFrom
 * blocking. The slave functionality uses interrupt routines for detecting start
 * and stop conditions and the overflow of the USI counter, which denotes a
 * transmitted byte or a (N)ACK
 *
 * This is a composed library of TinyWireM and TinyWireS, to provide
 * simultaneous slave and master configuration like the Wire library on an
 * Arduino. The original libraries were posted by BroHogan and modified by jkl
 * and the credit for writing the interface belongs to them.
 *
 * TODO:
 * - Reenable the random read function from TinyWireM (I don't really understand
...
#if defined(__AVR_ATmega165__) | defined(__AVR_ATmega325__) |                  \
