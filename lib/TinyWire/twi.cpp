...
// Receive ringbuffer with extra bytecounter, to give out the number of bytes
...
// Event function variables: called in case of a slave request or a slave
// receive
...
---------------------------------------------------------------*/
USI_TWI_MASTER_TRANSFER_RESULT USI_TWI_Master_Transfer(unsigned char temp,
                                                       bool ack) {
...
bool USI_TWI_Start_Read_Write(unsigned char *msg, unsigned char msgSize) {
  ...
  if (msgSize <= 1) // Test if the transmission buffer is empty
  {
    ...
    return (FALSE);
                     ...
                     // this execution.
  /*if( USISR & (1<<USISIF) ) // seems to always occur. Even, if nothing bad
  happened. Currently disabled
  {
  ...
  do {
    ...
    if (USI_TWI_state.addressMode || USI_TWI_state.masterWriteDataMode) {
      ...
      unsigned char error_code = USI_TWI_Master_Transfer(tempUSISR_8bit, false)
                                     .error_code; // Send 8 bits on bus.
      ...
      USI_TWI_MASTER_TRANSFER_RESULT temp_result =
          USI_TWI_Master_Transfer(tempUSISR_1bit, true);
      // No Check for Arbitration Lost, since the arbitration cannot be lost on
      // (N)ACK
    ...
    /* Else masterRead cycle*/
      ...
      DDR_USI &= ~(1 << PIN_USI_SDA); // Enable SDA as input.
      USI_TWI_MASTER_TRANSFER_RESULT temp_result =
          USI_TWI_Master_Transfer(tempUSISR_8bit, false);
...
void Twi_end() {
  ...
  DDR_USI &=
      ~((1 << PORT_USI_SCL) | (1 << PORT_USI_SDA)); // set SDA & SCL as input
...
void Twi_slave_init(uint8_t slave_addr) {
  ...
  GENERAL_INTERRUPT_MASK |=
      1 << PIN_CHANGE_INTERRUPT_ENABLE; // enable Pin Change Interrupt
  GENERAL_INTERRUPT_FLAGS |=
      1 << PIN_CHANGE_FLAG; // clear interrupt flag, by writing 1 to it
  PIN_CHANGE_MASK |=
      1 << PIN_USI_SDA; // enable pin change interrupt on PCINT0 (SDA line)
}
...
uint8_t Twi_slave_send(uint8_t *data, uint8_t length) {
  ...
  while ((count < length) && (Twi_slave_send(data[count]) == 1))
    count++;
...
*/
uint8_t Twi_available(void) { return rxByteNum; }
...
void Twi_master_init(void) {
  ...
  PORT_USI |=
      (1
       << PORT_USI_SCL); // Enable pullup on SCL, to set high as released state.
...
uint8_t Twi_master_send(uint8_t data) {
  ...
  if (temphead == txTail)
    return 0; // return 0 if buffer is full
...
uint8_t Twi_master_send(uint8_t *data, uint8_t length) {
  ...
  // send byte one by one as long as there's free space in the buffer
  while ((count < length) && (Twi_master_send(data[count]) == 1))
    count++;
...
uint8_t Twi_master_endTransmission() {
  ...
  if (twi_bus_busy) {
    USI_TWI_state.errorState = USI_TWI_BUS_BUSY;
    return USI_TWI_state.errorState;
  }

  if (USI_TWI_Start_Read_Write(tempbuf, j))
    return 0;
  else
...
uint8_t Twi_master_requestFrom(uint8_t slave_addr, uint8_t numBytes) {
  ...
  tempbuf[0] =
      (slave_addr << TWI_ADR_BITS) | USI_RCVE; // setup address & Rcve bit
  if (twi_bus_busy) {
    USI_TWI_state.errorState = USI_TWI_BUS_BUSY;
    return USI_TWI_state.errorState;
  }
  ...
  for (uint8_t i = 0; i < TWI_RX_BUFFER_SIZE;
       i++) { // swap data from tempbuffer to rxBuffer
...
ISR(USI_START_VECTOR) {
  ...
  if (!(PIN_USI & (1 << PIN_USI_SDA))) {
    ...
    if (currently_receiving) { // if we had received something, this is a
                               // RESTART condition and we have to call the
                               // users receive callback
...
ISR(USI_OVERFLOW_VECTOR) {
  ...
  // Address mode: check address and send ACK (and next USI_SLAVE_SEND_DATA) if
  // OK, else reset USI
  case USI_SLAVE_CHECK_ADDRESS:
    if ((USIDR == 0) || ((USIDR >> 1) == slaveAddress)) {
      if (USIDR & 0x01) {
        overflowState = USI_SLAVE_SEND_DATA;
        twi_onSlaveTransmit(); // call user event function for a request from
                               // master
      } else {
  ...
  case USI_SLAVE_CHECK_REPLY_FROM_SEND_DATA:
    if (USIDR) {
      ...
      SET_USI_TO_TWI_START_CONDITION_MODE();
      // clear TX buffer, because the rest of data in it is unwanted by the
      // master
      txHead = txTail = 0;
      return;
    }
...
// Interrupt Routine, triggered when SDA pin changes; for detecting Stop
// condition
ISR(PCINT0_vect) {
  if ((PIN_USI & (1 << PIN_USI_SDA)) && (PIN_USI & (1 << PIN_USI_SCL))) {
    ...
    if ((PIN_USI & (1 << PIN_USI_SDA)) && (PIN_USI & (1 << PIN_USI_SCL))) {
      twi_bus_busy = false;
      if (currently_receiving) { // If we are receiving bytes from a master,
                                 // call user callback
        if (rxByteNum > 0)
          twi_onSlaveReceive(
              rxByteNum); // check, if anything is in the rx buffer, because it
                          // is possible, that the communication was interrupted
                          // by a stop condition
        currently_receiving = false;
      }
